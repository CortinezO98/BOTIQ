...
export default function ChatWidget({ position = "bottom-right", primaryColor = C, embedded = false }) {
  ...
  const { messages, loading, session, sessionStatus, startSession, sendMessage, clearChat } = useChat();
  ...
  useEffect(() => {
    if (open) supportAPI.status().then((r) => setKbStatus(r.data)).catch(() => {});
  }, [open]);
  ...
  const content = (
    <div style={{
      width: embedded ? "100%" : 410,
      height: embedded ? "calc(100vh - 58px)" : 640,
      background: "#fff",
      ...
      <Header primaryColor={primaryColor} onReset={resetLocalFlow} onClose={() => setOpen(false)} showClose={!embedded} />

      {kbStatus && !kbStatus.drive_configured && messages.length === 0 && (
        <div style={{ background: "#fef3c7", borderBottom: "1px solid #fde68a", padding: "8px 16px", fontSize: 11, color: "#92400e" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {session && messages.length > 0 && <SessionBanner session={session} status={sessionStatus} />}
...
function ProfileSelector({ selectedProfile, networkUsername, setNetworkUsername, configureProfile, configureSupport, loading, error, primaryColor }) {
  return (
    <div style={{ textAlign: "center", padding: "22px 6px" }}>
      <div style={{ display: "flex", justifyContent: "center", marginBottom: 14 }}>
        <BotiqAvatar size={64} color={primaryColor} online />
      </div>
      <p style={{ fontWeight: 800, color: primaryColor, fontSize: 16, marginBottom: 6 }}>Antes de iniciar</p>
      <p style={{ color: "#6b6b8a", fontSize: 12, marginBottom: 16, lineHeight: 1.6 }}>
        Indícame tu perfil para configurar el flujo conversacional y controlar el uso de IA.
      </p>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        ...
        <button disabled={loading} onClick={() => document.querySelector('input[placeholder="ej: jose.cortez"]')?.focus()} style={profileBtn(primaryColor, selectedProfile === "support_engineer")}>
          🛠️<br />Ingeniero de Soporte
        </button>
      </div>

      <div style={{ marginTop: 14, textAlign: "left" }}>
        <label style={{ fontSize: 11, color: "#374151", fontWeight: 700 }}>Usuario de red para soporte</label>
        <input
      ...
      <div style={{ marginTop: 14, fontSize: 11, color: "#6b6b8a", lineHeight: 1.5 }}>
        Límite por sesión, validación de perfil y cierre automático ante temas ajenos al negocio.
      </div>

      {error && <div style={{ marginTop: 12, background: "#fef2f2", color: "#991b1b", border: "1px solid #fecaca", borderRadius: 10, padding: 10, fontSize: 12 }}>{error}</div>}
    </div>
  );
}
...
function SessionBanner({ session, status }) {
  ...
  return <div style={{ background: "#f5f5fa", border: "1px solid #e2e1f0", borderRadius: 12, padding: 10, fontSize: 11, color: "#374151" }}><strong>{profile}</strong> · Estado: <span style={{ color, fontWeight: 800 }}>{status}</span> · Máx. {session.max_questions} preguntas</div>;
}
  ...
  return (
    <div style={{ padding: "10px 14px", borderTop: `1px solid ${primaryColor}15`, background: "#fff" }}>
      <div style={{ display: "flex", alignItems: "flex-end", gap: 7 }}>
        ...
        <button disabled={disabled} onClick={() => fileRef.current?.click()} title="Adjuntar imagen" style={{ background: "none", border: "none", cursor: disabled ? "not-allowed" : "pointer", color: "#9ca3af", padding: 6, fontSize: 18 }}>📎</button>
        <textarea
          value={input}
          disabled={disabled}
        ...
        />
        <button onClick={send} disabled={disabled || loading || (!input.trim() && !imgFile)} style={{ background: disabled || loading || (!input.trim() && !imgFile) ? "#d1d5db" : `linear-gradient(135deg, ${primaryColor}, #3a3490)`, border: "none", borderRadius: "50%", width: 36, height: 36, cursor: disabled || loading || (!input.trim() && !imgFile) ? "not-allowed" : "pointer", color: "#fff" }}>▶</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "flex-start", flexDirection: isUser ? "row-reverse" : "row" }}>
      ...
      <div style={{ maxWidth: "78%" }}>
        <div style={{ background: isUser ? `linear-gradient(135deg, ${primaryColor}, ${primaryColor}dd)` : msg.meta?.isError ? "#fef2f2" : msg.meta?.system ? "#eef2ff" : "#f5f5fa", color: isUser ? "#fff" : msg.meta?.isError ? "#dc2626" : "#1a1a2e", borderRadius: isUser ? "14px 4px 14px 14px" : "4px 14px 14px 14px", padding: "10px 14px", fontSize: 13, lineHeight: 1.6, whiteSpace: "pre-wrap" }}>
          {msg.content}
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 4 }}>
          {mod && !isUser && <Meta color={mod.color}>{mod.icon} {mod.label}</Meta>}
          {msg.meta?.questionCount && <Meta>{msg.meta.questionCount}/{msg.meta.maxQuestions} preguntas</Meta>}
...
function Meta({ children, color = "#6b6b8a" }) { return <span style={{ fontSize: 10, color }}>{children}</span>; }
function Ava({ color, size, children }) { return <div style={{ width: size, height: size, borderRadius: "50%", background: color, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 12 }}>{children}</div>; }
function HBtn({ onClick, title, children }) { return <button onClick={onClick} title={title} style={{ background: "rgba(255,255,255,0.11)", border: "1px solid rgba(255,255,255,0.2)", color: "rgba(255,255,255,0.76)", width: 28, height: 28, borderRadius: 8, cursor: "pointer" }}>{children}</button>; }
function Typing() { return <div style={{ display: "flex", gap: 4, alignItems: "center", height: 14 }}>{[0,1,2].map((i)=><div key={i} style={{ width:5,height:5,borderRadius:"50%",background:"#9ca3af",animation:`tDot 1.2s ease-in-out ${i*0.2}s infinite`}}/>)}<style>{`@keyframes tDot{0%,80%,100%{transform:translateY(0)}40%{transform:translateY(-5px)}}`}</style></div>; }

function quickBtn(primaryColor) { return { background: `${primaryColor}08`, border: `1px solid ${primaryColor}25`, borderRadius: 20, padding: "7px 14px", fontSize: 12, cursor: "pointer", color: primaryColor, fontWeight: 600 }; }
function profileBtn(primaryColor, active) { return { background: active ? `${primaryColor}18` : "#f5f5fa", color: primaryColor, border: `1px solid ${active ? primaryColor : "#e2e1f0"}`, borderRadius: 14, padding: "14px 8px", cursor: "pointer", fontWeight: 800, fontSize: 12, lineHeight: 1.7 }; }
...
const closeImageBtn = { position: "absolute", top: -7, right: -7, background: "#dc2626", border: "none", borderRadius: "50%", width: 19, height: 19, cursor: "pointer", color: "#fff", fontSize: 11 };
